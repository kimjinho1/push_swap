...
/*   Updated: 2022/10/03 23:06:50 by jinhokim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "checker_bonus.h"

static void	check_op2(t_ps *ps, t_info *info, char *op)
{
	if (!ft_strncmp(op, "rra", 3) && ft_strlen(op) == 4)
		cmd_update(ps, 8, rra);
...
void	check_op(t_ps *ps, t_info *info, char *op)
{
	if (!ft_strncmp(op, "sa", 2) && ft_strlen(op) == 3)
		...
		cmd_update(ps, 6, rb);
	else if (!ft_strncmp(op, "rr", 2) && ft_strlen(op) == 3)
		cmd_update(ps, 7, rb);
	else
		check_op2(ps, info, op);
}
